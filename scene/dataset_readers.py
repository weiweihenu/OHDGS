...
from utils.general_utils import chamfer_dist, fov_to_intrinsics
import numpy as np
import json
import cv2
import math
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping):
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        K = fov_to_intrinsics(FovX, FovY, width, height)
        ...
        cam_info = CameraInfo(uid=uid, R=R, K=K, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
